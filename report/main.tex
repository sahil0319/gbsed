...
% ===========================================================================
% TODO BEFORE SUBMISSION: the appendix (weekly meeting log, GitHub commit/LoC
% metrics table, and the GitHub screenshots) is left unfilled intentionally --
% fill those in yourself before submission.
% ===========================================================================
...
\addauthor{Maisha Sanjida}{220041128}
\addauthor{Mahdi Islam}{220041149}
\addauthor{Saeera Tanjim}{220041107}

\supervisor{Md.\ Mahbub Alam}{Professor}{Department of Computer Science and Engineering}
\addcosupervisor{S.\ M.\ Sabit Bananee}{Lecturer}{Department of Computer Science and Engineering}
...
preceded by its length, and written as bytes. This flat serialisation is
byte-for-byte identical to the published implementation.
...
\section{Slice-Aligned Payload Packing}
...
slice did not arrive''. The capability existed; the flat serialisation of
Layer~3 discarded it, because a single vector cut into fixed-size chunks cannot
be parsed once a chunk is missing.

The slice-aligned layout builds the payload as a whole number of blocks of
exactly \texttt{chunkSize} bytes. Each block begins with a magic number and
carries only \emph{complete} relation slices; the first block additionally
carries the node block. Because the transport chunks naively at the same size,
every network chunk is one self-contained block. A missing block is all zeros,
fails the magic check and is skipped.

Slices are ordered safety-first. An earlier ordering that led with the
structural \texttt{isIn} relation pushed \texttt{near\_coll} into the second
block at small chunk sizes, where losing one chunk cost every safety relation
--- the exact failure the layout exists to prevent.
...
\begin{itemize}
    \item The flat serialisation remains the default and still round-trips
    byte-identically through the published serialiser for all 20 frames.
    \item The slice-aligned layout produces identical scene graphs to the flat
    serialisation (0 of 20 differ).
...
Because semantic decompression tolerates a partial relation set, a
reconstruction can succeed from only some of a frame's chunks --- but only if
the payload is packed to allow it. The flat serialisation cannot: a single
lost chunk leaves the concatenated vector unparseable, so the whole frame is
lost. The slice-aligned layout packs whole relation slices into independent,
self-describing blocks, ordered safety-first, so a lost chunk removes a
relation type rather than the frame.

At $500$-byte chunks, where frames genuinely span several chunks, this changes
the outcome. With identical bytes, identical delivery, and no additional chunks
on the wire at $1000$\,B (one additional at $500$\,B), safety-relation recall
under partial delivery improves as shown in Table~\ref{tab:graceful}.

\begin{table}[htbp]
\centering
\begin{tabular}{l c c}
\toprule
\textbf{Channel condition} & \textbf{Flat serialisation} & \textbf{Slice-aligned packing} \\
\midrule
Noise\_Low    & $0.80$ & $\mathbf{1.00}$ \\
Noise\_Medium & $0.50$ & $\mathbf{1.00}$ \\
\bottomrule
\end{tabular}
\caption{Safety-relation recall under partial delivery at a 500-byte chunk
size. Same bytes, same channel, same delivered frames.}
\label{tab:graceful}
\end{table}

A single frame illustrates the mechanism (Figure~\ref{fig:partial}). Frame~0005
of \texttt{seq1} occupies $1000$ bytes in two $500$-byte chunks; chunk~0 was
heard at $359$\,m and chunk~1 never arrived. With the slice-aligned layout the
delivered chunk carries the node block together with \texttt{super\_near},
\texttt{isIn} and \texttt{very\_near}, while the lost chunk carried the four
directional relations. The receiver therefore recovers the lane structure and
every safety-critical relation, losing only directional context.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/loss_resilience.png}
    \caption{Partial delivery of a single frame. Left: the transmitted graph.
    Right: the graph reconstructed after one of the two chunks was lost. The
    collision-risk relation \texttt{super\_near} rides in the delivered chunk
    and survives intact; only directional context is lost.}
    \label{fig:partial}
\end{figure}

Had the same chunk been lost under the flat serialisation, the truncated
payload would have been zero-filled, silently emptying whichever adjacency
matrices fell in the lost region --- here \texttt{super\_near} itself --- and
leaving only directional relations of no use to a braking decision. That
reconstruction would still contain exactly ten edges and score an identical
actor-edge F1 of $0.667$, yet preserve none of the safety-critical content.
\textbf{No aggregate structural metric distinguishes the two outcomes}, which
is precisely why safety-relation recall is reported as a separate metric in
this work. A second point follows: the flat serialisation not only loses the
safety relations but cannot detect that it has, since the zero-filled payload
parses without complaint and only a comparison against the sender's hash
reveals the damage.
...
\textbf{Integrity protection.} The payload carries no checksum, and the flat
serialisation parses a truncated payload without complaint. A CRC field in the
message would close this cleanly.
